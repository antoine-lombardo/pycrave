...
from .consts import ROOT_SCREEN_PAYLOAD, HEADERS, SEARCH_PAYLOAD, MEDIA_PAYLOAD, SEASON_PAYLOAD, SCREEN_PAYLOAD, ROOT_SCREENS, HOME_SCREEN,COLLECTION_PAYLOAD, GRID_PAYLOAD
...
class GraphQL():
  ...
  def get_elements(self, category: Category) -> List[Union[Category, ResultInfo]]:
    ...
    if category is None:
      ...
      return None
    logger.debug('Listing elements of category "{}"'.format(category.title))
  ...
  def get_elements_screen(self, id: str, root: bool = False) -> List[Union[Category, ResultInfo]]:
    ...
    response = self._make_request(payload, playback_language=self.metadata_language)
    ...
    try:
      if root:
        screens = json.loads(response.text)['data']['app']['navigationLinks']
      else:
        screens = json.loads(response.text)['data']['screen']['secondaryNavigation']['links']
      for screen in screens:
        try:
          ...
          if root and not screen['internalContent']['containerType'] in ROOT_SCREENS:
            continue
          elements.append(Category(type = 'screen', title = screen['linkLabel'], id = screen['internalContent']['id']))
        except:
          continue
    except:
    ...
    if root:
      ...
      return elements
    try:
      collections = json.loads(response.text)['data']['screen']['collections']
      if len(elements) == 0 and len(collections) == 1:
        if collections[0]['__typename'] == 'Grid': return self.get_elements_grid(collections[0]['id'])
        elif collections[0]['__typename'] == 'Rotator': return self.get_elements_rotator(collections[0]['id'])
        return None
      for collection in collections:
        try:
          if not 'config' in collection or not collection['config']['displayTitle']:
            continue
          if collection['__typename'] == 'Rotator': elements.append(Category(type = 'rotator', title = collection['title'], id = collection['id']))
          elif collection['__typename'] == 'Grid': elements.append(Category(type = 'grid', title = collection['title'], id = collection['id']))
        except:
          continue
    except:
  ...
  def get_elements_grid(self, id: str) -> List[Union[Category, ResultInfo]]:
    ...
    elements = []
    try:
      items = json.loads(response.text)['data']['grid']['collection']['page']['items']
      for item in items:
        try:
  ...
  def get_elements_rotator(self, id: str) -> List[Union[Category, ResultInfo]]:
    ...
    elements = []
    try:
      items = json.loads(response.text)['data']['rotator']['collection']['page']['items']
      for item in items:
        try:
          elements.append(self.parse_search_result(item))
        except:
